...
public class testUrl extends Init {
	...
	public void Start() throws InterruptedException {
		DriverManager.getDriver().navigate().to(Routers.SIGN_IN);
//		DriverManager.getDriver().merge(capabilities);

//		WebElement SubmitInBTN = Constants.getBtn("Yes, I understood");
//		SubmitInBTN.click();
		Thread.sleep(1000);

		WebElement SignInBTN = DriverManager.getDriver().findElement(By.xpath("//button[contains(text(),'Continue with email')]"));
//		WebElement SignInBTN = Constants.getBtn("Sign in");

		SignInBTN.click();
		sleep(4);
//        WebElement h1Tag = DriverManager.getDriver().findElement(By.className("hero__anchor"));
...
//        assertTrue(h1Tag.getText().contains("IB"));
	...
	public void TestAction() {
		...
		WebElement element = DriverManager.getDriver().findElement(By.xpath("//textarea[@id='APjFqb']"));
		// pass the product name that has to be searched in the website
		action.sendKeys(element, "Anh Tester")
				.build().perform();
		action.sendKeys(Keys.ENTER)
				.build().perform();
		DriverManager.getDriver().findElement(By.xpath("//a[@href='https://anhtester.com/']//h3[@class='LC20lb MBeuO DKV0Md'][normalize-space()='Anh Tester Automation Testing']"))
				.click();
		WebElement title = DriverManager.getDriver().findElement(By.xpath("//div[@class='col-lg-7']//h2[@class='section__title'][contains(text(),'Anh Tester')]"));
		action.doubleClick(title).build().perform();

		WebElement allCourseBTN = DriverManager.getDriver().findElement(By.xpath("//a[contains(text(),'Tất Cả Khóa Học')]"));
	...
	public void DrapAndDrop() throws AWTException {
		...
		DriverManager.getDriver().get("http://www.dhtmlgoodies.com/scripts/drag-drop-custom/demo-drag-drop-3.html");
		WebElement From = DriverManager.getDriver().findElement(By.xpath("//div[@id='box3']"));
		WebElement To = DriverManager.getDriver().findElement(By.xpath("//div[@id='box103']"));
